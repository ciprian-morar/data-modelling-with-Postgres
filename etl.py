...
index_songplay = 0
'''Column_data_types it\'s a dictionary of dictionaries used to store columns
data types for each of the tables in the Postgresql Database
...
def get_columns_data_types(cur):
    '''This function is used to get data and columns data types for each of the tables in the Postgresql Database
    and store it in local memory
    (dictionary columns_data_types)

    Parameters:
            cur (object): object cursor to execute queries
    Returns
            None
    '''
...
def data_quality_checks(table, df):
    ...
    Parameters:
            table (str): the name of the mapped table in the database
            df(pd.dataframe): the dataframe with data to insert in the table
    Returns
            df(pd.dataframe): returns the dataframe with the new columns data types.
    '''
    global columns_data_types

    '''
    check if the key column is primary key and if we have it in the dataframe(songplays has autoincrement
    implemented and songplay_id doesn't exist in df)
    ans = check_primary_key_df(df, table)'''
...
def process_song_file(cur, filepath, conn):
    ...
    Parameters:
            cur (object): object cursor to execute queries
            filepath (string): filepath of the file to open
...
def copy_from_stringio(conn, df, table, index_label, cur):
    ...
    Parameters:
            conn(object): Object Database connection
            df(pd.dataframe): dataframe with bulk data to insert in the table
            table(str): the name of the table where to insert the data
            index_label(string): column name of the primary key corespondent column
            cur (object): Object cursor to execute queries
    Returns
           None
    """
    ...
    df.to_csv(tmp_df, index=False, header=False, sep="\t", na_rep='NULL')
    f = open(tmp_df, 'r')
...
def process_log_file(cur, filepath, conn):
    ...
    songplay_df = pd.DataFrame(columns=['songplay_id', 'start_time', 'user_id', 'level', \
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = {'songplay_id': index_songplay, 'start_time': row.ts, 'user_id':row.userId, 'level':row.level, \
                         'song_id':songid, 'artist_id':artistid, 'session_id':row.sessionId, 'location':row.location, \
...
def process_data(cur, conn, filepath, func):
    ...
    Parameters:
            cur (object): object cursor to execute queries
            filepath (string): filepath of the main directory
            conn(object): object Database connection
            func(function): callback function to execute for processing the files
    Returns
           None
    """
